...
<span class="text-gray-700 font-semibold cursor-pointer group-hover:text-primary transition-colors text-sm pl-2 sidebar-text plava-boja">Promjena lozinke</span>
            </Link>
...
<div class="bg-white shadow-sm px-6 ml-16 rounded-xl">
        ...
        <div class="grid moj-grid-cols-3 grid-cols-change-password p-6 mb-4 gap-y-4 items-center gap-x-6 pr-4">
          ...
          <div class="text-sm text-right border-plava-200 broder-1 text-black inter my-auto">Trenutna lozinka</div>
          <div :class="{'flex-1 w-full': !isSidebarExpanded, 'flex-1 w-3/4 md:w-3/4': isSidebarExpanded}">
  <password v-model="passwordNow" type="text" />
</div>
<div class="text-sm text-right border-plava-200 broder-1 text-black inter my-auto">Nova lozinka</div>
<div :class="{'flex-1 w-full': !isSidebarExpanded, 'flex-1 w-3/4 md:w-3/4': isSidebarExpanded}">
  <password v-model="passwordNew" type="text" />
</div>
<div class="text-sm text-right border-plava-200 broder-1 text-black inter my-auto col-start-1">Potvrdi lozinku</div>
<div :class="{'flex-1 w-full': !isSidebarExpanded, 'flex-1 w-3/4 md:w-3/4': isSidebarExpanded}">
  <password v-model="passwordConfirmation" type="text" />
</div>

          <div class="col-start-2 col-span-1  ">
            <button :disabled="$v.$invalid" class="bg-primary1 font-medium px-6 py-2 rounded-md text-white text-sm filter hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50" @click="changePassword">Spremi</button>
          </div>
        </div>
      </div>
    </div>

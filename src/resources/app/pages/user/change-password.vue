<template>
  ...
  <!-- Floating Action Button -->
    ...
    <div class="pt-5  container grid grid-cols-1 md:grid-cols-profile " @wheel="handleScrollAttempt">  
     <!--1-->
...
<div class="bg-white shadow-sm px-6 ml-16 rounded-xl">
          <!--2-->
          <div class="h-22 flex flex-col py-4 justify-center border-plava-200 border-b">
            <span class="text-lg font-medium plavi-text">{{ $t('promijeni-lozinku') }}</span>
          </div>
        ...
        <div class="grid moj-grid-cols-3 grid-cols-change-password p-6 mb-4 gap-y-4 items-center gap-x-6 pr-4">
          <div class="w-full col-span-full">
            <alert closeable v-if="isSuccess">
              ...
              {{ $t('lozinka-je-uspjesno-promijenjena') }}
            </alert>
          </div>
          <div class="text-sm text-right border-plava-200 broder-1 text-black inter my-auto">{{ $t('trenutna-lozinka') }}</div>
          <div :class="{'flex-1 w-full': !isSidebarExpanded, 'flex-1 w-3/4 md:w-3/4': isSidebarExpanded}">
  <password v-model="passwordNow" type="text" />
</div>
<div class="text-sm text-right border-plava-200 broder-1 text-black inter my-auto">{{ $t('nova-lozinka') }}</div>
<div :class="{'flex-1 w-full': !isSidebarExpanded, 'flex-1 w-3/4 md:w-3/4': isSidebarExpanded}">
  <password v-model="passwordNew" type="text" />
</div>
<div class="text-sm text-right border-plava-200 broder-1 text-black inter my-auto col-start-1">{{ $t('potvrdi-lozinku') }}</div>
<div :class="{'flex-1 w-full': !isSidebarExpanded, 'flex-1 w-3/4 md:w-3/4': isSidebarExpanded}">
  <password v-model="passwordConfirmation" type="text" />
</div>

          <div class="col-start-2 col-span-1  ">
            <button :disabled="$v.$invalid" class="bg-primary1 font-medium px-6 py-2 rounded-md text-white text-sm filter hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50" @click="changePassword">{{ $t('spremi') }}</button>
          </div>
        </div>
      </div>
    </div>
    ...
    changePassword() {
      this.$api.skijasiUser
        ...
        .then(res => {
          ...
          this.isSuccess = true
          this.$helper.alert(this.$t('uspjesno'));
        })
        .catch(error => {

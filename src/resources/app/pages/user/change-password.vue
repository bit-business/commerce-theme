    ...
    <div class="pt-8 navbar container hidden sm:grid grid-cols-profile">
      <div class="pr-8">
        ...
        <div class="mt-4 flex items-center gap-y-3 flex-wrap">
          ...
          <Link :href="route('skijasi.commerce-theme.change-password')" class="w-full inline-flex items-center ml-10 group">
            <div class="col-start-2 col-end-3 cursor-pointer text-sm text-primary">Promijeni Lozinku</div>
          </Link>
      ...
      <div class="bg-white shadow-sm px-6 rounded-xl">
        <div class="h-22 flex flex-col py-4 justify-center border-b">
          <span class="text-lg font-medium text-gray-700">Promijeni Lozinku</span>
          <span class="text-sm text-gray-500">Radi sigurnosti vašeg računa, nemojte dijeliti svoju lozinku s drugima.</span>
        </div>
        <div class="grid grid-cols-change-password p-6 mb-4 gap-y-4 items-center gap-x-6 pr-4">
          ...
          <div class="text-sm text-right text-gray-500 my-auto">Trenutna lozinka</div>
          <div class="flex-1 w-3/4">
            <password v-model="passwordNow" type="text" />
          </div>
          <div class="text-sm text-right text-gray-500 my-auto">Nova lozinka</div>
          <div class="flex-1 w-3/4">
            <password v-model="passwordNew" type="text" />
          </div>
          <div class="text-sm text-right text-gray-500 my-auto col-start-1">Potvrdi lozinku</div>
          <div class="flex-1 w-3/4">
            <password v-model="passwordConfirmation" type="text" />
          </div>
          <div class="col-start-2 col-span-1">
            <button :disabled="$v.$invalid" class="bg-primary font-medium px-6 py-2 rounded-md text-white text-sm filter hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50" @click="changePassword">Spremi</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="sm:hidden relative">
      <div class="bg-white py-2 flex flex-col">
        <div class="py-1 border-b">
          <input type="password" class="py-2 outline-none text-sm px-3 w-full" placeholder="Unesite trenutnu lozinku" v-model="passwordNow">
        </div>

        <div class="py-1 border-b">
          <input type="password" class="py-2 outline-none text-sm px-3 w-full" placeholder="Unesi novu lozinku" v-model="passwordNew">
        </div>

        <div class="py-1">
          <input type="password" class="py-2 outline-none text-sm px-3 w-full" placeholder="Potvrdite novu lozinku" v-model="passwordConfirmation">
        </div>

        <div class="p-3 pb-1">
          <div class="bg-primary w-full text-white text-center py-2 text-sm rounded-md" @click="changePassword">Spremi</div>
        </div>
      </div>
    </div>
  </div>

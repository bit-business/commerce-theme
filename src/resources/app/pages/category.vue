      ...
      <div class="grid lg:grid-cols-6 lg:gap-4 md:grid-cols-4 md:gap-2 w-full h-full">
        ...
        <div class="col-start-1 row-span-full flex flex-wrap gap-6 mb-auto">
          ...
          <div class="flex flex-wrap gap-3">
            <Link v-if="activeCategory" :href="route('skijasi.commerce-theme.category', $page.props.slug)" class="text-primary w-full font-bold text-sm flex items-center px-2 relative pl-4">
            ...
            <Link :href="route('skijasi.commerce-theme.category', category.slug)" v-for="category, index in filteredCategories" :key="index" class="text-gray-500 w-full font-medium text-sm flex items-center px-2 pl-4 relative">
              {{ category.name }}
            </Link>
          </div>
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    next(vm => {
      document.title = `${vm.$voca.titleCase(vm.$page.props.slug, true)} Category - Skijasi Commerce Theme`
    })
  },
  mounted() {

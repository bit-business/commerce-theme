<template>
  <div class="w-full h-72 hover:shadow-md rounded-xl transition-transform duration-200 ease-in-out transform hover:-translate-y-1" v-if="product.productDetails.length > 0">
    <Link :href="route('skijasi.commerce-theme.detalji', product.slug)" class="relative block group bg-white rounded-xl h-full" >
      ...
      <div class="absolute right-4 top-0 h-8 w-8" v-if="hasActiveDiscount">
        <span class="absolute right-0 top-0 text-xs z-10 w-full text-center text-white font-bold line-clamp-2">-{{ getDiscount }}<br/>
          <span class="text-white font-normal">POPUSTA</span>
        </span>

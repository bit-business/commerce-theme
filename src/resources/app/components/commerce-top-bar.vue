  ...
  <div class="relative select-none bg-primary flex items-stretch w-full h-8">
    <div class="container flex">
      <div class="flex flex-shrink-0 items-center divide-x">
        <a href="https://github.com/nadzorservera-croatia/skijasi" target="_blank" class="flex-grow-0 flex-shrink-0 relative px-4 leading-normal text-white no-underline flex items-center hover:text-gray-300 text-sm">
          Skijasi
        </a>
        <a href="https://github.com/nadzorservera-croatia/skijasi-commerce-module" target="_blank" class="flex-grow-0 flex-shrink-0 relative px-4 leading-normal text-white no-underline flex items-center hover:text-gray-300 text-sm">
          Commerce Module
        </a>
      </div>
        ...
        <div class="md:flex md:justify-end ml-auto items-center">
          <div class="flex-grow-0 flex-shrink-0 relative px-4">
            <div class="block w-full h-full pointer-events-auto relative z-20" @mouseover="dropdownOpen = true">
              <Link :href="route('skijasi.commerce-theme.notification')" class="flex leading-normal text-white no-underline items-center hover:text-gray-300 text-sm">
                ...
                <div class="border shadow-xl border-gray-300 bg-white rounded-sm navigation-arrow-top">
                  <div class="p-2">
                    <template v-if="isAuthenticated">
                      ...
                      <Link :href="route('skijasi.commerce-theme.notification')" class="flex items-start px-2 py-2 hover:bg-gray-100 -mx-2" v-for="notification, index in notifications" :key="index">
                    ...
                    <template v-else>
                      <div class="h-16 flex justify-center items-center flex-col">
                        ...
                        <Link :href="route('skijasi.commerce-theme.login')" class="block text-primary rounded-md text-sm font-medium">Log In</Link>
                      </div>
                    </template>
                  </div>
                  <Link v-if="notifications.length > 5" :href="route('skijasi.commerce-theme.notification')" class="block bg-white text-gray-700 text-center text-sm py-2 -mt-2 hover:bg-gray-100">Tampilkan Semua</Link>
                </div>
              </div>
            </transition>
          </div>
          ...
          <div class="flex items-center divide-x">
            <template v-if="!isAuthenticated">
              <Link :href="route('skijasi.commerce-theme.register')" class="flex-grow-0 flex-shrink-0 relative px-3 leading-normal text-white no-underline flex items-center hover:text-gray-300 text-sm font-medium">
                Daftar
              </Link>
              <Link :href="route('skijasi.commerce-theme.login')" class="flex-grow-0 flex-shrink-0 relative px-3 leading-normal text-white no-underline flex items-center hover:text-gray-300 text-sm font-medium">
                Login
              </Link>
            </template>
            <template v-else>
              <Link :href="route('skijasi.commerce-theme.profile')" class="flex-grow-0 flex-shrink-0 relative px-4 leading-normal text-white no-underline flex items-center hover:text-gray-300 text-sm font-medium cursor-pointer">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">

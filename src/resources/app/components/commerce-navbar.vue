        ...
        <div class="flex items-stretch justify-end mx-auto flex-grow-0 flex-shrink-0 relative px-4">
            <div class="block w-full h-full pointer-events-auto relative z-30" @mouseover="dropdownOpen = true">
              <template v-if="isAuthenticated">
                <Link :href="route('skijasi.commerce-theme.cart')" :class="['flex flex-grow-0 flex-shrink-0 relative md:px-0 lg:py-2 lg:px-4 leading-normal text-white no-underline items-center hover:text-gray-300 text-sm']">
              ...
              <template v-else>
                <Link :href="route('skijasi.commerce-theme.login')" :class="['flex flex-grow-0 flex-shrink-0 relative md:px-0 lg:py-2 lg:px-4 leading-normal text-white no-underline items-center hover:text-gray-300 text-sm']">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.85" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                  </svg>
                </Link>
